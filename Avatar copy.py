...
class EmotionState:
    ...
    def process_behavior(self, behavior, psycho_cunsumption, success_value):
...
class VirtualAgent:
    ...
    def apply_behavior_feedback(self, behavior_name, success_value):
        behavior = self.current_behavior[behavior_name]
        ...
        self.bio_energy += bio_cunsumption * dot_bio_weight
        ...
        # 情绪反馈：调用情绪状态更新
        self.emotion.process_behavior(behavior, psycho_cunsumption * dot_psycho_social_weight, success_value)
    ...
    def _select_behavior(self, current_Time_slot="morning",life_style_weight = None):
        ...
        #这里要添加外部事件，更新behavior_library
        self.current_behavior = {}
        for k, v in self.behavior_library.items():
            self.current_behavior[k] = v.copy() # 深拷贝（防止后续修改影响原始字典）

        with open("M_file/External_Event.json", "r", encoding="utf-8") as f:
            external_events = json.load(f)

        for k, v in external_events.items():
            self.current_behavior[k] = v  # 允许新事件覆盖原有同名事件

        for name, content in self.current_behavior.items():
            ...
            emotion_arousal = 0.5  # 若未来扩展可替换此默认值

            valence = np.clip(emotion_valence, -1.0, 1.0)
            arousal = np.clip(emotion_arousal, 0.0, 1.0)

            emotion_modifier = 1.0 + 0.25 * valence + 0.25 * (arousal - 0.5)
    ...
    def _apply_behavior(self, chosen_behavior, success_rate=0.7):
        ...
        for_user = self.current_behavior[chosen_behavior].get("for_user", 0.0)
        if for_user == 1.0:
            success = 1.0
        ...
        # 随机选择 Detail 描述（如有）
        behavior_data = self.current_behavior.get(chosen_behavior, {})
    ...
    def print_state(self):
        ...
        print("🔋 情绪效价:" + self.emotion.current_mood_label())
        dim_names = [
